...
@registry.register_sensor
class NormalizedTaskStateSensor(Sensor):
    """
    Normalized sensor providing compact task state.
    Returns robot-relative positions normalized to [-1, 1] range.
    
    Output shape: (9,) containing:
    - [0:3]: Object position relative to EE (normalized)
    - [3:6]: Target zone position relative to robot base (normalized)
    - [6:9]: EE position relative to robot base (normalized)
    
    This replaces the redundant 24-dim observation from ColorZoneSensor + ObjectColorSensor.
    """
    cls_uuid: str = "normalized_task_state"
    
    def __init__(self, sim: Simulator, config, *args, **kwargs):
        self._sim = sim
        self._normalization_range = config.get("normalization_range", 2.0)  # ±2m workspace
        super().__init__(config=config)
    ...
    def _normalize_position(self, pos: np.ndarray, reference: np.ndarray) -> np.ndarray:
        """Normalize position relative to reference to [-1, 1] range."""
        relative = pos - reference
        normalized = relative / self._normalization_range
        return np.clip(normalized, -1.0, 1.0)
    
    def get_observation(self, observations, episode, *args, **kwargs):
        # Default observation (all zeros if something fails)
        ...
        try:
            ...
            # Get robot positions
            robot_base = np.array(agent.base_pos)
            ee_transform = agent.ee_transform()
            ee_pos = np.array(ee_transform.translation)
            
            # Get task zones
            task = episode.task if hasattr(episode, "task") else None
            target_zone_pos = np.array([0.0, 0.8, 0.0])  # Default
            
            if task and hasattr(task, "_color_zones"):
                # For single-object task, use red zone (color_id=0)
                if "red" in task._color_zones:
                    zone_data = task._color_zones["red"]
                    zone_pos = zone_data.get("position")
                    if zone_pos is not None:
                        target_zone_pos = np.array(zone_pos)
            
            # Get object position (first red_cube found)
            rigid_obj_mgr = self._sim.get_rigid_object_manager()
            object_pos = ee_pos.copy()  # Default to EE if no object
            
            for obj_handle in rigid_obj_mgr.get_object_handles():
                if "red_cube" in obj_handle or "cubeSolid" in obj_handle:
                    obj = rigid_obj_mgr.get_object_by_handle(obj_handle)
                    object_pos = np.array(obj.translation)
                    break  # Use first object found
            
            # Normalize observations
            # [0:3] Object relative to EE
            obs[0:3] = self._normalize_position(object_pos, ee_pos)
            
            # [3:6] Target zone relative to robot base
            obs[3:6] = self._normalize_position(target_zone_pos, robot_base)
            
            # [6:9] EE relative to robot base
            obs[6:9] = self._normalize_position(ee_pos, robot_base)
            
        except Exception as e:
            # Fail gracefully - return zeros
            pass
